...
export class VideosPage implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.loginUserData) {
      ...
      this.postService.getGuestPostById({ 'postId': this.postID }).subscribe((data) => {
        console.log(data);
        this.postData = data.postData;
        this.postUserName = data.postData.post_user.user_name;
        ...
        if(this.videoCount){
          ...
          localStorage.setItem('videosWatched','1');
        }
        ...
        if (this.type != 'exclusive') {
          this.commonService.presentLoader();
          this.peopleView.guestUpNextPost('2', this.postUserId, this.videoType, this.postID).subscribe((data) => {
            this.upNext = data.postData;
            this.commonService.dismissLoader();
            if (this.upNext.length < 1) {
              this.noData = true;
