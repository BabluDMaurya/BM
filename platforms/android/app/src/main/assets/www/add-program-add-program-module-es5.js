...
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["add-program-add-program-module"], {
  ...
  function srcAppAddProgramAddProgramPageTs(module, __webpack_exports__, __webpack_require__) {
    ...
    "./src/app/add-program/videos-thumb-list/videos-thumb-list.component.ts"); // import { VideoEditor } from '@ionic-native/video-editor/ngx';
      ...
      _createClass(AddProgramPage, [{
          ...
          if (this.timeSlot == true) {
            if (this.newEvent.events.length > 0) {
              if (this.noEvent == true) {
                this.newEvent.events.forEach(function (el) {
                  console.log(_this11.hours);
                  console.log(el.startTime.getHours());

                  _this11.dateObj.setHours(_this11.hours, parseInt(_this11.minutes));

                  if (el.startTime.getHours() == _this11.hours || el.endTime.getHours() == _this11.hours) {
                    console.log(el.startTime.getTime() + 'event time');
                    _this11.noEvent = false;
                    console.log(el.startTime.getTime());
                    console.log(_this11.dateObj.getTime());
                    console.log(el.endTime); // if ( (el.startTime.getTime() > this.dateObj.getTime() && el.startTime.getTime() >= nd.getTime()) || (el.endTime.getTime() <= this.dateObj.getTime())) {
                    ...
                    if (el.startTime.getTime() > _this11.dateObj.getTime() || el.endTime.getTime() <= _this11.dateObj.getTime()) {
                      _this11.noEvent = true;
                      console.log('newww');
                    } else {
                      _this11.noEvent = false;

                      _this11.selectNewTime();
            ...
            if (this.noEvent == true) {
              ...
              this.selectDate.push(this.dateObj); // this.selectDate.forEach(el => {
            ...
            } else {// this.selectTimeSlot();
              // this.selectNewTime();
            }
          }

          console.log(this.timeSlot);
          console.log(this.noEvent); // else{
          //   this.selectTimeSlot();
          // } 
          // this.repetatedDate = this.repetatedDateCopy;
        }
                        ...
                        handler: function handler(data) {
                          ...
                          if (data.duration <= 0) {
                            ...
                            if (_this13.noEvent == true) {
                              ...
                              // this.progEndTime = nd;
                              _this13.selectDate.push(_this13.dateObj);

                              _this13.repetatedDateCopy.push({
                                'date': _this13.dateObj,
                                'equipments': [],
                                'nutrition_id': [],
                                'video': '',
                                'description': ''
                              ...
                              _this13.repetatedDate.push(_this13.dateObj);

                              _this13.timeSlot = true; // this.repetatedDate = this.repetatedDateCopy;
                            }
                          }
                        }
                      }]
              ...
              while (1) {
                switch (_context9.prev = _context9.next) {
                  ...
                  case 2:
                    ...
                    modal.onDidDismiss().then(function (d) {
                      if (d.data) {
                        _this14.repetatedDateCopy[i].video = d.data; // this.repetatedDate[i].video=d.data;
                      }
                    });
                    _context9.next = 6;
                    return modal.present();
              ...
              while (1) {
                switch (_context10.prev = _context10.next) {
                  ...
                  case 2:
                    ...
                    modal.onDidDismiss().then(function (d) {
                      if (event == 1) {
                        _this15.repetatedDateCopy[i].equipments = d.data.filter(Boolean); // this.repetatedDate[i].equipments = d.data.filter(Boolean) ;
                      } else {
                        _this15.repetatedDateCopy[i].nutrition_id = d.data.filter(Boolean); // this.repetatedDate[i].nutrition_id = d.data.filter(Boolean) ;
                      }

                      console.log(_this15.repetatedDateCopy); // console.log(this.repetatedDate);
                    });
                    _context10.next = 6;
                    return modal.present();
        ...
        value: function selectNewTime() {
          ...
          var buttons = [{
            ...
            text: 'Confirm',
            handler: function handler(value) {
              _this16.noEvent = true;
              var hours = value.Hours.value;
              console.log(hours); // this.dateObj.setMinutes(hours);

              console.log(_this16.dateObj); // var newEndTime = new Date(this.eventList.selectedTime + 'Z');

              var newDateTime = _this16.dateObj; // this.dateObj.setHours((this.hours),  parseInt(this.minutes));

              var nd = newDateTime.setHours(_this16.hours, parseInt(hours));
              _this16.progEndTime = nd;
              console.log(_this16.progEndTime);
              console.log(nd);

              _this16.newEvent.events.forEach(function (el) {
                if (el.startTime.getHours() == _this16.dateObj.getHours() || el.endTime.getHours() == _this16.dateObj.getHours()) {
                  console.log(el.startTime.getTime());
                  console.log(el.endTime.getTime());
                  console.log(_this16.dateObj.getTime());

                  if (el.startTime.getTime() > _this16.dateObj.getTime() && el.startTime.getTime() >= _this16.progEndTime.getTime() || el.endTime.getTime() <= _this16.dateObj.getTime()) {
                    _this16.noEvent = true;
                    _this16.timeSlot = true;
                  } else {
                    _this16.commonService.presentToast('this time slot already has event');

                    _this16.noEvent = false;
                  }
                }
              });

              _this16.repetatedDateCopy.forEach(function (el) {
                if (el.date.getDate() == _this16.dateObj.getDate()) {
                  _this16.noEvent = false;
                  return;
              ...
              if (_this16.noEvent == true) {
                // this.repetatedDateCopy.push(this.dateObj);
                _this16.selectDate.push(_this16.dateObj);

                _this16.repetatedDate.push(_this16.dateObj);

                _this16.repetatedDateCopy.push({
                  'date': _this16.dateObj,
                  'equipments': [],
                  'nutrition_id': [],
                  'video': '',
                  'description': ''
              ...
              hours = '';
              console.log(_this16.repetatedDateCopy); //  alert(this.noEvent)
            }
          }];
          this.commonService.presentItemPicker(defaultColumnOptions, buttons);
        }
                ...
                switch (_context11.prev = _context11.next) {
                  ...
                  case 2:
                    ...
                    modal.onDidDismiss().then(function (d) {
                      ...
                      if (_this17.isVideoSelected == true) {
                        var url = baseUrl + "api/auth/uploadVideoForProgram";

                        var filename = _this17.selectedVideo.substr(_this17.selectedVideo.lastIndexOf('/') + 1);
                        ...
                        _this17.videoFileUpload = _this17.transfer.create();

                        _this17.videoFileUpload.upload(_this17.selectedVideo, url, options).then(function (data) {
                          console.log('gg');
                      ...
                      if (d.data.program_date) {
                        ...
                        _this17.programDetail = d.data;

                        _this17.ionViewWillEnter();

                        _this17.showProgram = 3;
                        _this17.ProgramInserted = true;
                      }
                    });
                    _context11.next = 6;
                    return modal.present();
        ...
        value: function pickImage(sourceType) {
          ...
          this.camera.getPicture(options).then(function (imageData) {
            _this18.gallaryImgPath.push('data:image/jpeg;base64,' + imageData);
          }, function (err) {
            alert(err);
          });
        ...
        value: function pickVideoFromGallery(sourceType) {
          ...
          this.camera.getPicture(options).then(function (videoUrl) {
            ...
            var dur = _this20.media.create(dirpath);
            ...
            _this20.selectedVideoFile(dirpath, filename); // this.gallaryImgPath.push('data:image/jpeg;base64,' + imageData);
        ...
        value: function searchChanged(musicname) {
          ...
          if (searchTerm == '') {
            this.musicService.getGenres().subscribe(function (data) {
              _this22.genres = data.genres;
              _this22.allMusic = data.genres[0].musics;
            });
          } else {
            this.programService.searchMusic({
              "searchQuery": searchTerm
            }).subscribe(function (search) {
              _this22.allMusic = search.musicList;
            });
          }
        }
      }, {
        ...
        value: function prepareAudioFile() {
          ...
          this.platform.ready().then(function () {
            _this23.musicChanged = 1;
            _this23.audioFile = _this23.baseUrl + "public/storage/audio/" + _this23.selectedAudioFile;
          });
        }
      }, {
        key: "toLastScreen",
    ...
    tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([Object(_angular_core__WEBPACK_IMPORTED_MODULE_1__["ViewChild"])(_calendar_calendar_component__WEBPACK_IMPORTED_MODULE_7__["CalendarComponent"], {
      "static": true
    }), tslib__WEBPACK_IMPORTED_MODULE_0__["__metadata"]("design:type", _calendar_calendar_component__WEBPACK_IMPORTED_MODULE_7__["CalendarComponent"])], AddProgramPage.prototype, "myCal", void 0);
      ...
      _createClass(AudioPlayerProgramComponent, [{
        ...
        value: function _bindPlayerEvents() {
          ...
          this._player.addEventListener('timeupdate', function () {
            _this25.currentTime = Math.floor(_this25._player.currentTime);
          });
          ...
          this._player.addEventListener('loadeddata', function () {
            _this25.isLoading = false;
            var minutes = Math.floor(_this25._player.duration / 60);
            _this25.duration = minutes + ':' + Math.floor(_this25._player.duration - minutes * 60);
          });
        }
      }]);
